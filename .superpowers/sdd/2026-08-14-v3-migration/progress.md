...
Task 7: complete (commit 0c6707b). save_figS2_data rewritten: serial top-N trial re-run via calib.build_fn/run_sim; cancers-by-age from `s.analyzers['by_age'].to_dataframe('cancers').iloc[0].values`; genotype dists from `hpv.results_by_genotype(s, key='cum_cancers'|'n_cin', normalize=True).loc[2015.0].values`; target CSVs from __file__-relative data/ paths. _load_calib_data in run_sims.py updated to scoped col names and bare-df return. Smoke: 1 passed in 87s serial. All 6 CSVs produced.
